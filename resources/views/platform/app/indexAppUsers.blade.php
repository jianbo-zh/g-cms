                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <i class="fa fa-align-justify"></i> 平台应用用户列表
                        </div>
                        <div class="card-body">
                            <form method="get" action="{{ route('indexAppUsers', ['appId'=>$appId]) }}" class="form-inline table-search">
                                ...
                                <div class="form-actions">
                                    ...
                                    <a href="{{ route('createAppUser', ['appId'=>$appId]) }}" class="btn btn-success">新增</a>
                                </div>
                            </form>
                            <table class="table table-responsive-sm table-bordered">
                                ...
                                @foreach($users as $user)
                                    <tr>
                                        ...
                                        <td>
                                            @if($user['id'] !== 1)
                                                <a href="{{ route('editAppUser', ['appId'=>$appId, 'userId'=>$user['id']]) }}" class="btn btn-primary">编辑</a>
                                                <button class="btn btn-danger btn-ladda ladda-button destroyAppUser" data-style="zoom-out" data-id="{{ $user['id'] }}">刪除</button>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
...
@push('scripts')
    <script>
        $(function(){
            $('.destroyAppUser').click(function () {
                let userId = $(this).attr('data-id');
                let $that = $(this);
                if(confirm('确认删除该用户？')){
                    let ladda = Ladda.create(this).start();
                    let turl = '{{ route('destroyAppUser', ['appId'=>$appId, 'userId'=>'[uid]']) }}';
