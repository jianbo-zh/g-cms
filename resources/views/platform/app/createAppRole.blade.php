                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <i class="fa fa-align-justify"></i> 创建应用角色
                        </div>
                        <div class="card-body">
                            <form method="post" class="form-horizontal" id="createAppRoleForm">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-6">
                                        <textarea rows="3" class="row-span form-control" id="description" name="description"></textarea>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-6">
                                        <label class="switch switch-label switch-outline-primary-alt">
                                            <input type="checkbox" class="switch-input" checked="" name="state" value="1">
                                                                    ...
                                                                    <div class="col-form-label">
                                                                        @foreach($group['perms'] as $key3 => $perm)
                                                                            <div class="form-check form-check-inline mr-1">
                                                                                <input class="form-check-input" type="checkbox" name="perms[]" id="inline-checkbox-{{ $loop->parent->parent->index }}-{{ $loop->parent->index }}-{{ $loop->index }}" value="{{ $key3 }}">
                        ...
                        <div class="card-footer">
                            <div class="row">
                                ...
                                <div class="form-actions col-md-10">
                                    <button class="btn btn-primary btn-ladda ladda-button" data-style="zoom-out" id="createAppRoleSubmit">
...
<script>
    $(function () {
        ...
        $('#createAppRoleSubmit').click(function () {
            let formData = $('#createAppRoleForm').serializeJSON();
            let ladda = Ladda.create(this).start();
            z_ajax('post', '{{ route("storeAppRole", ['appId'=>$appId]) }}', formData, function (response) {
                ladda.stop();
                let data = response.data;
                if(data.code === 0){
                    z_notify_success(data.message ? data.message : '操作成功！', function () {
                        location.href = '{{ route("indexAppRoles", ['appId'=>$appId]) }}';
                    });
                }else{
