                        ...
                        <div class="card-footer">
                            <div class="row">
                                ...
                                <div class="form-group-actions col-md-9">
                                    <button class="btn btn-success btn-ladda ladda-button" data-style="zoom-out" id="updateContentUserSubmit">
...
@push('scripts')
    <script>
        $(function(){
            $('#updateContentUserSubmit').click(function(){
                let ladda = Ladda.create(this).start();
                let formData = $('#updateContentUserForm').serializeJSON();

                z_ajax('put', '{{ route('updateContentUser', ['appId'=>$appId, 'userId'=>$user['id']]) }}', formData, function (response) {
                    let data = response.data;
                    if(data.code === 0){
                        z_notify_success(data.message ? data.message : '操作成功！', function () {
                            location.href = '{{ route('indexContentUsers', ['appId'=>$appId]) }}';
                        });
                    }else{
